...
function getBottomNav(currentPage) {
  const navItems = [
    { path: '/', name: 'home', icon: '🏠', label: 'Home', isHome: true },
  ...
  const navHtml = navItems.map(item => {
    ...
    const homeAttr = item.isHome ? ' data-nav-home="1"' : '';
    return '<a href="' + item.path + '" class="nav-item' + (active ? ' active' : '') + '" data-path="' + item.path + '" data-name="' + item.name + '"' + homeAttr + ' aria-current="' + (active ? 'page' : 'false') + '">' +
      '<span class="nav-item-icon" aria-hidden="true">' + item.icon + '</span>' +
      '<span class="nav-item-label">' + item.label + '</span>' +
    '</a>';
  }).join('');

  return '<nav class="bottomnav app-bottom-nav" role="navigation" aria-label="Main navigation">' + navHtml + '</nav>';
}
