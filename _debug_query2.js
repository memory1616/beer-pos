...
conn.on('ready', () => {
  ...
  // Test the exact pattern used in the query
  const sql = "SELECT COALESCE(SUM(reward_liters_used), 0) as total FROM sales WHERE archived = 0 AND promo_type = 'MONTHLY_BONUS' AND reward_liters_used > 0 AND (note LIKE '%tháng ' || 6 || '%/%' || 2026 || '%');";
  ...
  conn.exec('echo "' + encoded + '" | base64 -d > /tmp/q.sql && sqlite3 /root/beer-pos/database.sqlite < /tmp/q.sql && rm /tmp/q.sql', (err, stream) => {
    ...
    stream.on('end', () => {
      ...
      conn.exec('echo "' + encoded2 + '" | base64 -d > /tmp/q2.sql && sqlite3 /root/beer-pos/database.sqlite < /tmp/q2.sql && rm /tmp/q2.sql', (err2, stream2) => {
        ...
        stream2.on('end', () => {
          console.log('Matching sales:', out2);
          conn.end();
        });
      });
    });
