...
/**
 * Lấy keg balance của khách hàng
 * @param {number} customerId
 * @returns {number}
 */
function getCustomerKegBalance(customerId) {
  const customer = db.prepare('SELECT keg_balance FROM customers WHERE id = ?').get(customerId);
  return customer?.keg_balance || 0;
}

/**
 * Lấy lịch sử keg của khách hàng từ ledger
 * @param {number} customerId
 * @param {number} limit
 * @returns {Array}
 */
function getCustomerKegHistory(customerId, limit = 20) {
  return db.prepare(`
    SELECT * FROM keg_ledger
    WHERE customer_id = ?
...
/**
 * Thống kê keg theo khách hàng
 * @returns {Array}
 */
function getKegStatsByCustomer() {
  return db.prepare(`
    SELECT
      c.id as customer_id,
    ...
    FROM customers c
    LEFT JOIN keg_ledger l ON l.customer_id = c.id
    WHERE c.archived = 0 AND c.keg_balance > 0
    GROUP BY c.id
    ORDER BY c.keg_balance DESC
  `).all();
}

module.exports = {
  updateCustomerKegBalanceTx,
  getCustomerKegBalance,
  getCustomerKegHistory,
  getKegStatsByCustomer
};
