...
const SalesStaffService = {
  ...
  getProductCommissions() {
    try {
      const result = db.prepare(`
        SELECT spc.*, p.name as product_name, p.type as product_type
  ...
  addProductCommission(productId, salaryPerLiter, productType = 'all') {
    ...
    const existing = db.prepare(`
      ...
      WHERE (product_id = ? OR (product_id IS NULL AND ? IS NULL))
        AND product_type = ?
    `).get(productId || null, productId || null, productType);
